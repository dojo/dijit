...
//TODO: ok to change this form postCreate to buildRendering?  Need attachpoints defined earlier
	buildRendering: function(){
		...
		if(dojo.isIE || dojo.isSafari){
			...
			if(!w || !this.iframe.contentDocument.title){
				this.iframe.buildRendering = dojo.hitch(this, this.buildRendering);
				return;
			}
			var d = w.document;
	...
	_onIframeBlur: function(){
		...
		this.iframe.contentDocument.title = this.iframeEditTitle;
	},
	...
	_onKeyPress: function(e){
		if(e.keyCode == dojo.keys.TAB && !e.shiftKey && !e.ctrlKey && !e.altKey && this.iframe){
			...
			// eliminated.
			this.iframe.contentDocument.title = this.iframeFocusTitle;
