define([
	...
	"./EnterKeyHandling",
	"../../form/ToggleButton"
], function(declare,array,lang,domAttr,domClass,domConstruct,listTraverse,domStyle,has,query,dijit,_Plugin,rangeapi,EnterKeyHandling,ToggleButton){

	// module:
	//		dijit/_editor/plugins/BidiSupport
	
	var BidiSupport = declare("dijit._editor.plugins.BidiSupport", _Plugin, {
		// summary:
		...
		//		and its behaviuor is controlled both by buttons and by shortcut (Ctrl+Shift+X). 
		...
		setEditor: function(/*dijit.Editor*/ editor){
			...
			//		witch creates problems for further editing of the text.
			//	Expand native command
			...
			this.editor.onLoadDeferred.addCallback(lang.hitch(this, function(){
				var edPlugins = this.editor._plugins,
					i, p, f = false,
					h = lang.hitch(this,this.handleChangeDir);	
				
				this.editor.addKeyHandler('x', 1, 1, h); //Ctrl-Shift-X
				
				for(i = 0; i < edPlugins.length; i++){
					p = edPlugins[i];
					if(p && (p.declaredClass === "dijit._editor.plugins.EnterKeyHandling" ||
							p.declaredClass === (dijit._scopeName + "._editor.plugins.EnterKeyHandling"))){							
						if(!p.blockNodeForEnter || p.blockNodeForEnter == "BR"){
							p.changeBlockNodeFromBrToDiv();
						}
						this.blockMode = p.blockNodeForEnter;
						f = true;
						break;
					}
				}
				if(!f){
					...
					this.editor.addPlugin({name: 'dijit._editor.plugins.EnterKeyHandling', blockNodeForEnter: this.blockMode});
				}
		...
		handleChangeDir: function(){
			// summary:
			//		Called in result of using Ctrl+Shift+X shortcut.
			//		Starts execution of 'mirror' (change text direction to opposite) command. 		
			this._changeState("mirror");
		},
		...
		_execNativeCmd: function(cmd, arg, info){
			if(this._isSimpleInfo(info)){
				...
				if(has("webkit")){
					query("table",this.editor.editNode).prev().forEach(lang.hitch(this,function(x,ind,arr){
			...
			for(var i = 0; i < info.nodesInfo.groups.length; i++){
				var group = info.nodesInfo.groups[i];
				...
				if(has("webkit")){
					if(table){
						if(table.previousSibling && table.previousSibling.tagName && table.previousSibling.tagName.toUpperCase() == "BR"){
					...
					}else if(i == info.nodesInfo.groups.length-1){
		...
		_indentAndOutdent: function(cmd){
			...
			var info;
			if(cmd == "outdent"){
				info = this._changeState("prepareoutdent");
			}else{
				info = this._changeState("prepareindent");
			}
		...
		_changeState: function(cmd,arg){
			...
			return {nodesInfo: nodesInfo, range: tempRange};
		},
		...
		_changeStateOfBlocks: function(startNode, endNode, commonAncestor, cmd, arg, tempRange){
			...
			var nodesInfo = {nodes: nodes, groups: info.groups, cells: info.cells, 
					range: {startContainer: origStartContainer, startOffset: origStartOffset, endContainer: origEndContainer, endOffset: origEndOffset}};
			...
			switch(cmd.toString()){
				...
				case "insertunorderedlist":
					this._execInsertLists(nodesInfo, cmd, arg);
					break;
				...
				case "normalizeoutdent":
					this._execIndentAndOutdent(nodesInfo, cmd, arg);
					break;
				//before 'formatblock' native command
				case "prepareformat":
				//after executing 'formatblock' native command
				case "formatblock":
					this._execFormatBlocks(nodesInfo, cmd, arg);
					break;				
				default: console.error("Command " + cmd.toString() + " isn't handled");
			}
		...
		_execDirAndAlignment: function(nodesInfo, cmd,arg){
			...
			case "rtl":
				...
				query("table",this.editor.editNode).forEach(lang.hitch(this,function(table,idx,array){
					var dir = cmd;
					if(cmd === "mirror"){
		...
		_execInsertLists: function(nodesInfo, cmd, arg){
			// summary:
			//		Perform changes before and after native insertorderedlist and 
			//		insertunorderedlist commands for each node from the given array.
			switch(cmd.toString()){
			//before executing 'insert list' native command
			case "preparelists":
				var nodesChaged = false;
				array.forEach(nodesInfo.nodes, function(x,index,arr){
					...
					if(has("mozilla") || has("webkit")){
						...
						if(has("webkit") && this._changeListType(x, arg) && x === x.parentNode.lastChild){
						...
						if(this._hasTag(x,"LI")){
							...
							if(this._getLILevel(x) == 1 && !this._changeListType(x,cmd)){
				...
				if(has("webkit")){
					query("table",this.editor.editNode).forEach(lang.hitch(this,function(x,ind,arr){
			...
			case "insertunorderedlist":
				...
				if(has("mozilla")){
					query("*[tempRole]",this.editor.editNode).forEach(lang.hitch(this,function(x,index,arr){
		...
		_execIndentAndOutdent: function(nodesInfo, cmd, arg){
			// summary:
			//		Perform changes before and after native indent and outdent commands 
			//		for each node from the given array.		
			switch(cmd.toString()){
			case "normalizeindent":
				array.forEach(nodesInfo.nodes, function(x){
		...
		_execFormatBlocks: function(nodesInfo, cmd, arg){
			// summary:
			//		Perform changes before and after native formatblock command 
			//		for each node from the given array.				
			switch(cmd.toString()){
			//before 'formatblock' native command
			case "prepareformat":
				array.forEach(nodesInfo.nodes, function(x){
					//In some cases Mozill'a native 'formatblock' command mistakely merges
					//contents of list items. Problem can be solved by 'manual' formatting.
					if(has("mozilla")){
						if(this._hasTag(x,"LI")){
					...
					if(has("webkit")){
						//Chrome and Safari remove styles of reformatted blocks
						var styledSpan;
						if(this._hasTag(x,"LI")){
							var savedName = arg;
				...
				array.forEach(nodesInfo.nodes, function(x){
					if(this._hasTagFrom(x, this._lineTextArray)){
						...
						if(this._hasTag(x.parentNode,"DIV") && x.parentNode !== this.editor.editNode){
							while(x.parentNode.childNodes.length > 1){
		...
		_refineLIMargins: function(node){
			...
			//var table = this.editor.selection.getParentOfType(node,["TABLE"]);
			if(has("webkit")){
		...
		_hasTagFrom: function(node,arr){
			if(!(node && node.tagName && arr)){
				return false;
			}
			var re = new RegExp("^(?:" + arr.join("|") + ")$");
			return re.test(node.tagName.toUpperCase());
		},
		...
		_isSimpleInfo: function(info){
			return !info || info.nodesInfo.groups.length < 2;
		},
		
		_changeListType: function(node, cmd){
			if(!this._hasTag(node,"LI")){
				return false;
			}
			var parent = node.parentNode;
		...
		_mergeLists: function(){
			query("ul,ol",this.editor.editNode).forEach(lang.hitch(this,function(x,ind,arr){
				if(domAttr.has(x,"tempRole")){
					x.parentNode.removeChild(x);
					return;
				}
		...
		_cleanLists: function(){
			if(has("webkit")){
				query("table", this.editor.editNode).forEach(lang.hitch(this,function(x,ind,arr){
					var sibling = x.nextSibling;
					if(sibling && sibling.nodeType == 1 && sibling.tagName.toUpperCase() === "UL" && domAttr.get(sibling,"tempRole") === "true"){
						sibling.parentNode.removeChild(sibling);
					}
				}));
