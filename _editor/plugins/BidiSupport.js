define([
	...
	"./EnterKeyHandling",
	"../../form/ToggleButton"
], function(declare,array,aspect,lang,domAttr,domClass,domConstruct,listDom,listTraverse,domStyle,has,query,dijit,_Plugin,rangeapi,EnterKeyHandling,ToggleButton){
	...
	var BidiSupport = declare("dijit._editor.plugins.BidiSupport", _Plugin, {
		...
		setEditor: function(/*dijit.Editor*/ editor){
			...
			//		wich creates problems for further editing of the text.
			//	Expand native command
			...
			this.editor.onLoadDeferred.addCallback(lang.hitch(this, function(){
				...
				for(i = 0; i < edPlugins.length; i++){
					p = edPlugins[i];
					if(p && p.constructor === EnterKeyHandling){						
				...
				if(!f){
					...
					this.editor.addPlugin({ctor: EnterKeyHandling, blockNodeForEnter: this.blockMode});
				}
			}));
		...
		_cleanLists: function(){
			...
			var wasMoved = false;
			query("span[bogusDir]", this.editor.editNode).forEach(lang.hitch(this,function(x,ind,arr){
